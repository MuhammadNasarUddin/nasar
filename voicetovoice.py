...
@rehanaiv3app.route('/')
def home():
    return render_template('voicetovoice.html')
...
def listen():
    try:    
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system",
                     "content": "You are a helpful assistant of Rehan Ai. Your job is to provide accurate answers to any questions asked by users. If a user asks a question in English, you will respond in English, and if in Urdu, then in Urdu. If I don't know the answer, you will indicate that I don't know."},
...
@rehanaiv3app.route('/process_audio', methods=['POST'])
def process_audio():
    try:
...
@rehanaiv3app.route('/static/<filename>')
def serve_static(filename):
    return send_from_directory(rehanaiv3app.static_folder, filename)
